...
3. [Đọc hiểu hình ảnh](#3-đọc-hiểu-hình-ảnh)
4. [Vòng đời một robot](#4-vòng-đời-một-robot)
5. [Các tập tính & thuật toán](#5-các-tập-tính--thuật-toán)
6. [Kết quả kiểm chứng](#6-kết-quả-kiểm-chứng)
7. [Bảng tham số](#7-bảng-tham-số)
8. [Kiến trúc mã nguồn](#8-kiến-trúc-mã-nguồn)
...
## 5. Các tập tính & thuật toán

### 5.1 Điều hướng — Trường thế nhân tạo
...
### 5.3 Phân việc đấu giá thị trường (market-based) + **chống lưỡng lự**
...
> [§6](#6-kết-quả-kiểm-chứng)); robot dư không còn chen lấn cản đường đội đang làm.

### 5.4 Thông tin băng tải — Cảm nhận cục bộ + tin đồn lan truyền
...
### 5.5 Stigmergy — Né đông bãi đỗ qua "mùi" trên sàn
...
### 5.6 Quản lý năng lượng

Pin thật của ARGoS (`time_motion`), **ngưỡng cứng** không dự đoán khoảng cách:
...
### Tác động của việc sửa lưỡng lự (5 seed, cấu hình chuẩn 10 robot)
...
### Chi tiết an toàn (5 seed sau khi sửa)
...
### Khả năng mở rộng (5 và 30 robot, giữ nguyên 10 ô sạc cố định)
...
### Năng lượng & stigmergy

| Tham số | Mặc định | Ý nghĩa |
|---|---|---|
| `<battery> full_charge`/`start_charge` | 6.0 | Dung lượng ×6 mặc định ARGoS; thời gian sạc giữ nguyên |
| `<battery>` `pos_factor`/`time_factor` | 0.008 / 2e-5 | Hao pin theo mét / theo tick |
...
│   ├── warehouse_robot_update.cpp         #  1 vòng/tick: sạc+dock+stigmergy, metric, giao hàng
...
thống kê). Loop functions gọi vào controller qua vài hàm API (`SetBeltGroundTruth`,
...
- [x] ~~Trạm sạc + quản lý pin~~ · ~~gossip thay oracle~~ · ~~stigmergy né đông~~ ·
  ~~test quy mô 5–30 robot~~ · ~~chống lưỡng lự~~ — **đã xong**.
