...
mọi benchmark giữ nguyên (đã kiểm chứng bit-for-bit sau khi thêm console).
...
> [§7](#7-kiểm-chứng-throughput-an-toàn-và-mở-rộng)); robot dư không còn chen lấn cản đường đội đang làm.
...
### 7.1 Trước / sau khi diệt lưỡng lự (5 seed, cấu hình chuẩn 10 robot)
...
### 7.2 Chi tiết an toàn (5 seed sau khi sửa)
...
- [x] ~~Trạm sạc + quản lý pin~~ · ~~sửa bug NaN pin~~ · ~~gossip thay oracle~~ · ~~stigmergy né đông~~ ·
  ~~test quy mô 5–30 robot~~ · ~~diệt lưỡng lự~~ · ~~bảng điều khiển vận hành~~ — **đã xong**.
